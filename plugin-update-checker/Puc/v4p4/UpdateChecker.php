...
if ( !class_exists('Puc_v4p4_UpdateChecker', false) ):
	...
	abstract class Puc_v4p4_UpdateChecker {
		...
		/**
		 * Check if $result is a successful update API responses.
		 *
		...
		protected function validateApiResponse($result) {
			...
			if ( !isset($result['responses']['code']) ) {
				return new WP_Error(
			...
			if ( $result['responses']['code'] !== 200 ) {
				return new WP_Error(
					'puc_unexpected_response_code',
					'HTTP responses code is ' . $result['responses']['code'] . ' (expected: 200)'
				);
			}
